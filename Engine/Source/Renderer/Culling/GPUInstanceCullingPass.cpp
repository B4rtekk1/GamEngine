...
namespace Engine::Culling {
    ...
    void GPUInstanceCullingPass::record(const VkCommandBuffer commandBuffer,
        ...
        // The visibility list is consumed immediately by grass compute
        // passes, then by the vertex shader during indirect rendering.
        // Synchronize both the IDs and their atomic counter for both uses.
        const VkBufferMemoryBarrier2 outputBarriers[] = {
            {.sType = VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER_2,
             ...
             .buffer = m_visibleInstances, .offset = 0, .size = VK_WHOLE_SIZE},
            {.sType = VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER_2,
             .srcStageMask = VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT,
             .srcAccessMask = VK_ACCESS_2_SHADER_STORAGE_WRITE_BIT,
             .dstStageMask = VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT | VK_PIPELINE_STAGE_2_VERTEX_SHADER_BIT,
        ...
        const VkDependencyInfo outputDependency{.sType = VK_STRUCTURE_TYPE_DEPENDENCY_INFO,
            .bufferMemoryBarrierCount = 2, .pBufferMemoryBarriers = outputBarriers};
