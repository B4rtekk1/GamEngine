...
VkDeviceAddress AccelerationStructureManager::meshAddress(const void* key) const noexcept { const auto i = blases_.find(key); return i == blases_.end() ? 0 : i->second.address; }
