...
struct MainSettings {
    ...
    MainSettings(glm::vec2 projection, float radius, float falloffValue, std::uint32_t frame, float pad)
        : projectionScale(projection), radiusView(radius), falloff(falloffValue), frameIndex(frame), padding(pad),
...
VkPipeline makeCompute(VkDevice device, Assets::AssetManager& assets, const char* path, VkPipelineLayout layout) {
...
void GtaoPass::create(VkPhysicalDevice physical, VkDevice device, VkExtent2D fullExtent, VmaAllocator allocator,
    ...
    try {
        ...
        depthPipelines_[0] = makeCompute(device_, assets, "shaders/gtao_linearize_depth.spv", depthPipelineLayouts_[0]);
        depthPipelines_[1] =
        ...
        for (uint32_t p = 0; p < 3; ++p) {
            ...
            for (uint32_t i = 0; i < inputCount[p]; ++i)
                b.push_back({i, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1, VK_SHADER_STAGE_COMPUTE_BIT, nullptr});
...
void GtaoPass::buildLinearDepth(VkCommandBuffer cmd, uint32_t frame, VkImageView depth, VkSampler depthSampler,
    ...
    for (uint32_t mip = 1; mip < linearDepthMipCount_; ++mip) {
        ...
        vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, depthPipelineLayouts_[1], 0, 1, &set, 0, nullptr);
...
void GtaoPass::record(VkCommandBuffer cmd, uint32_t frame, uint32_t sampleIndex, VkImageView depth,
                      VkSampler depthSampler, VkImageView viewNormal, VkSampler viewNormalSampler, bool useExternalNormals, const Mat4& inverseProjection) {
    ...
    update(0, {{linearDepthSampler_, linearDepthView_, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL}, {viewNormalSampler, viewNormal, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL}},
    ...
    barrier(cmd, raw_.image(), VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_LAYOUT_GENERAL, VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT,
