...
void BloomPass::create(VkPhysicalDevice physical, VkDevice device, VkExtent2D sourceExtent,
    ...
    try {
        ...
        VkDescriptorPoolSize size{VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, FramesInFlight}; VkDescriptorPoolCreateInfo pi{VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO}; pi.maxSets = FramesInFlight; pi.poolSizeCount = 1; pi.pPoolSizes = &size;
        if (vkCreateDescriptorPool(device_, &pi, nullptr, &pool_) != VK_SUCCESS) throw std::runtime_error("Could not create bloom pool");
        std::array<VkDescriptorSetLayout, FramesInFlight> layouts{}; layouts.fill(layout_);
        VkDescriptorSetAllocateInfo ai{VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO}; ai.descriptorPool = pool_; ai.descriptorSetCount = FramesInFlight; ai.pSetLayouts = layouts.data();
        if (vkAllocateDescriptorSets(device_, &ai, sets_.data()) != VK_SUCCESS) throw std::runtime_error("Could not allocate bloom sets");
    } catch (...) { destroy(); throw; }
}
void BloomPass::record(VkCommandBuffer commandBuffer, VkImageView source, VkSampler sampler, const std::uint32_t frameIndex) {
