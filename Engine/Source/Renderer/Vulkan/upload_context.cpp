...
UploadTicket UploadContext::Batch::submit() { if (context_ == nullptr) return {}; UploadContext* context = std::exchange(context_, nullptr); return context->submit(); }
...
void UploadContext::abort() noexcept { if (!commandBuffer_) return; vkEndCommandBuffer(commandBuffer_); vkFreeCommandBuffers(device_, pool_, 1, &commandBuffer_); commandBuffer_ = VK_NULL_HANDLE; }
