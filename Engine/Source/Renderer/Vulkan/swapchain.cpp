...
namespace Engine {
    namespace {
        struct SwapchainSupport {
            // Vulkan writes every member of this output structure.
        ...
        VkCompositeAlphaFlagBitsKHR chooseCompositeAlpha(
            ...
            for (VkCompositeAlphaFlagBitsKHR candidate: candidates) {
                if ((supported & static_cast<VkCompositeAlphaFlagsKHR>(candidate)) != 0) {
                    return candidate;
                }
            }
    ...
    void Swapchain::createResources(VkSwapchainKHR oldSwapchain) {
        ...
        if ((support.capabilities.supportedUsageFlags &
             static_cast<VkImageUsageFlags>(VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT)) == 0) {
