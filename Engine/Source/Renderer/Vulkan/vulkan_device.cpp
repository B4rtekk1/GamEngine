...
namespace Engine {
    ...
    QueueFamilyIndices VulkanDevice::findQueueFamilies(VkPhysicalDevice candidate) const {
        ...
        for (uint32_t i = 0; i < queueFamilyCount; i++) {
            ...
            if (const VkResult result = vkGetPhysicalDeviceSurfaceSupportKHR(candidate, i, surface_, &supportsPresent);
                result == VK_SUCCESS && supportsPresent == VK_TRUE) {
                indices.present = i;
            }
        }
    ...
    void VulkanDevice::selectPhysicalDevice(const VkInstance candidate) {
        ...
        memoryBudgetExtensionSupported_ = std::ranges::any_of(extensions,
                                                              [](const VkExtensionProperties &extension) {
                                                                  return std::strcmp(extension.extensionName,
                                                                                     VK_EXT_MEMORY_BUDGET_EXTENSION_NAME) == 0;
                                                              });
        meshShaderExtensionSupported_ = std::ranges::any_of(extensions,
                                                            [](const VkExtensionProperties &extension) {
                                                                return std::strcmp(extension.extensionName,
