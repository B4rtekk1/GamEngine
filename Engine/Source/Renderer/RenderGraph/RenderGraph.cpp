...
namespace Engine::RenderGraph {
    ...
    void RenderGraph::exportTexture(const TextureHandle texture) {
        ...
        if (std::ranges::find(exportedTextures_, texture) == exportedTextures_.end()) {
            exportedTextures_.push_back(texture);
        }
    }
    ...
    void RenderGraph::addBufferAccess(const std::uint32_t pass, const BufferHandle buffer, const BufferUsage usage,
        ...
        if (bufferUsageInfo(usage).write != write) {
            throw std::invalid_argument("Buffer usage does not match read/write declaration");
        }
    ...
    void RenderGraph::compile() {
        ...
        for (const auto &pass: passes_) {
            ...
            for (const auto &[texture, usage, write]: pass.accesses) {
                mix(texture.index);
                mix(static_cast<std::uint8_t>(usage));
                mix(static_cast<std::uint64_t>(write));
            }
            for (const auto &[buffer, usage, write]: pass.bufferAccesses) {
                mix(buffer.index);
                mix(static_cast<std::uint8_t>(usage));
                mix(write);
            }
        ...
        if (cachedTopology != topologyCaches_.end()) {
            ...
            for (std::uint32_t pass = 0; pass < count; ++pass) {
                for (const Access &access: passes_[pass].accesses) {
                    ...
                    if (!access.write) {
                        if (lastWriter[resource] >= 0) { edges[static_cast<std::uint32_t>(lastWriter[resource])].insert(
                            pass);
                ...
                for (const BufferAccess &access: passes_[pass].bufferAccesses) {
                    ...
                    if (!access.write) {
                        if (lastBufferWriter[resource] >= 0) { edges[static_cast<std::uint32_t>(lastBufferWriter[
                            resource])].insert(pass);
                        } else if (!buffers_[resource].imported) throw std::logic_error(
            ...
            topologyCaches_.push_back({.signature = signature, .order = order_, .valid = true});
        }
        ...
        if (passCullingEnabled_ && (!exportedTextures_.empty() || !exportedBuffers_.empty())) {
            ...
            for (std::uint32_t pass = 0; pass < count; ++pass) {
                for (const auto &access: passes_[pass].accesses) {
                    ...
                    if (!access.write) {
                        ...
                        addDependency(writer[resource], pass);
                        for (const auto reader: readers[resource]) { addDependency(
                ...
                for (const auto &access: passes_[pass].bufferAccesses) {
                    ...
                    if (!access.write) {
                        ...
                        addDependency(bufferWriter[resource], pass);
                        for (const auto reader: bufferReaders[resource]) { addDependency(
                            static_cast<std::int32_t>(reader), pass);
}
                        bufferReaders[resource].clear();
            ...
            for (const auto texture: exportedTextures_) { if (writer[texture.index] >= 0) { pending.push_back(
                static_cast<std::uint32_t>(writer[texture.index]));
}
}
            for (const auto buffer: exportedBuffers_) { if (bufferWriter[buffer.index] >= 0) { pending.push_back(
        ...
        for (std::uint32_t pass = 0; pass < count; ++pass) {
            for (const auto &access: passes_[pass].accesses) {
                const auto resource = access.texture.index;
                if (!access.write) {
                    if (dependencyWriter[resource] >= 0) { addQueueEdge(dependencyWriter[resource], pass);
}
                    dependencyReaders[resource].push_back(pass);
                } else {
                    if (dependencyWriter[resource] >= 0) { addQueueEdge(dependencyWriter[resource], pass);
}
                    for (const auto reader: dependencyReaders[resource]) { addQueueEdge(reader, pass);
}
            ...
            for (const auto &access: passes_[pass].bufferAccesses) {
                const auto resource = access.buffer.index;
                if (!access.write) {
                    if (dependencyBufferWriter[resource] >= 0) { addQueueEdge(dependencyBufferWriter[resource], pass);
}
                    dependencyBufferReaders[resource].push_back(pass);
                } else {
                    if (dependencyBufferWriter[resource] >= 0) { addQueueEdge(dependencyBufferWriter[resource], pass);
}
                    for (const auto reader: dependencyBufferReaders[resource]) { addQueueEdge(reader, pass);
}
        ...
        for (const auto &dependency: queueDependencies_) {
            ...
            if (auto &waits = queueBatches_[consumer].waitBatches; std::ranges::find(waits, producer) == waits.end()) { waits.push_back(producer);
}
        }
        ...
        for (std::uint32_t resource = 0; resource < buffers_.size(); ++resource) {
            if (!buffers_[resource].imported && buffers_[resource].lifetime.firstPass != count) { transientBuffers.
                    push_back(resource);
}
}
    ...
    void RenderGraph::allocateTransientBuffers(const std::vector<BufferDesc> &slotDescs) {
        ...
        try {
            for (const BufferDesc &desc: slotDescs) {
                const auto reusable = std::find_if(transientBufferAllocations_.begin(),
                                                   transientBufferAllocations_.end(),
