...
namespace Engine {
    ...
    void Skybox::create(VkPhysicalDevice physicalDevice, VkDevice device, VkCommandPool commandPool, VkQueue queue,
        ...
        try {
            ...
            pool.maxSets = static_cast<uint32_t>(uniformBuffers.size());
            pool.poolSizeCount = 2;
            pool.pPoolSizes = sizes;
            if (vkCreateDescriptorPool(device_, &pool, nullptr, &descriptorPool_) != VK_SUCCESS) {
                throw std::runtime_error("Could not create skybox descriptor pool");
            }
            ...
            if (vkAllocateDescriptorSets(device_, &allocation, descriptorSets_.data()) != VK_SUCCESS) {
                throw std::runtime_error("Could not allocate skybox descriptor sets");
            }
    ...
    void Skybox::draw(VkCommandBuffer commandBuffer, uint32_t frameIndex) const {
        ...
        vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline_.layout(), 0, 1,
                                &descriptorSets_.at(frameIndex), 0, nullptr);
    ...
    void Skybox::destroy() noexcept {
        if (device_ != VK_NULL_HANDLE && descriptorPool_ != VK_NULL_HANDLE) { vkDestroyDescriptorPool(
