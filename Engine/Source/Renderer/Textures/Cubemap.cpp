...
namespace Engine {
    namespace {
        uint32_t findMemoryType(VkPhysicalDevice physicalDevice, uint32_t typeFilter,
            ...
            for (uint32_t i = 0; i < memoryProperties.memoryTypeCount; ++i) {
                if (((typeFilter & (1U << i)) != 0U) && (memoryProperties.memoryTypes[i].propertyFlags & properties) ==
                    properties) {
                    return i;
                }
            }
        ...
        void submitAndWait(VkDevice device, VkQueue queue, VkCommandBuffer commandBuffer) {
            ...
            if (vkCreateFence(device, &fenceInfo, nullptr, &fence) != VK_SUCCESS) {
                throw std::runtime_error(
                    "Could not create cubemap upload fence");
            }
            VkSubmitInfo submit{VK_STRUCTURE_TYPE_SUBMIT_INFO};
            submit.commandBufferCount = 1;
            submit.pCommandBuffers = &commandBuffer;
            const VkResult submitted = vkQueueSubmit(queue, 1, &submit, fence);
            const VkResult completed = submitted == VK_SUCCESS
                                           ? vkWaitForFences(device, 1, &fence, VK_TRUE, UINT64_MAX)
                                           : submitted;
            vkDestroyFence(device, fence, nullptr);
            if (completed != VK_SUCCESS) {
                throw std::runtime_error("Could not upload cubemap");
            }
        }
    }
    ...
    void Cubemap::createHdr(VkPhysicalDevice physicalDevice, VkDevice device, VkCommandPool commandPool,
        ...
        if (faceSize == 0 || mipLevels == 0 || mipLevels > static_cast<uint32_t>(std::bit_width(faceSize))) {
            throw
                    std::invalid_argument("Cubemap has invalid dimensions or mip count");
        }
        ...
        if (rgbaPixels.size() != expectedFloats) {
            throw std::invalid_argument("Cubemap HDR data has an invalid size");
        }
        ...
        try {
            ...
            if (upload != nullptr) {
                ...
                const auto [buffer, offset, mapped] = upload->allocate(imageSize, 16);
                staging = buffer;
                stagingOffset = offset;
                std::memcpy(mapped, halfPixels.data(), static_cast<size_t>(imageSize));
                ...
                if (vkCreateBuffer(device_, &bufferInfo, nullptr, &staging) != VK_SUCCESS) {
                    throw std::runtime_error(
                        "Could not create cubemap staging buffer");
                }
                ...
                stagingAlloc.memoryTypeIndex = findMemoryType(physicalDevice, stagingRequirements.memoryTypeBits,
                                                              VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                                                              VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
                if (vkAllocateMemory(device_, &stagingAlloc, nullptr, &stagingMemory) != VK_SUCCESS) {
                    throw
                            std::runtime_error("Could not allocate cubemap staging memory");
                }
                if (vkBindBufferMemory(device_, staging, stagingMemory, 0) != VK_SUCCESS) {
                    throw std::runtime_error(
                        "Could not bind cubemap staging memory");
                }
                void *mapped = nullptr;
                if (vkMapMemory(device_, stagingMemory, 0, imageSize, 0, &mapped) != VK_SUCCESS) {
                    throw
                            std::runtime_error("Could not map cubemap staging memory");
                }
                std::memcpy(mapped, halfPixels.data(), static_cast<size_t>(imageSize));
            ...
            if (vkCreateImage(device_, &imageInfo, nullptr, &image_) != VK_SUCCESS) {
                throw std::runtime_error(
                    "Could not create cubemap image");
            }
            VkMemoryRequirements requirements{};
            vkGetImageMemoryRequirements(device_, image_, &requirements);
            ...
            if (vkAllocateMemory(device_, &alloc, nullptr, &memory_) != VK_SUCCESS) {
                throw std::runtime_error(
                    "Could not allocate cubemap image memory");
            }
            if (vkBindImageMemory(device_, image_, memory_, 0) != VK_SUCCESS) {
                throw std::runtime_error(
                    "Could not bind cubemap image memory");
            }
            ...
            if (upload == nullptr) {
                ...
                if (vkAllocateCommandBuffers(device_, &commandAlloc, &commandBuffer) != VK_SUCCESS) {
                    throw
                            std::runtime_error("Could not allocate cubemap upload command buffer");
                }
            ...
            regions.reserve(static_cast<std::size_t>(mipLevels_) * 6);
            VkDeviceSize offset = 0;
            for (uint32_t mip = 0; mip < mipLevels_; ++mip) {
                const auto side = std::max(1U, faceSize >> mip);
                const VkDeviceSize faceBytes = static_cast<VkDeviceSize>(side) * side * 4 * sizeof(uint16_t);
                for (uint32_t face = 0; face < 6; ++face) {
                    VkBufferImageCopy copy{};
                    copy.bufferOffset = stagingOffset + offset;
            ...
            vkCmdCopyBufferToImage(commandBuffer, staging, image_, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
            ...
            if (upload != nullptr) {
                ...
                if (ownsUploadBatch) {
                    readyTimeline_ = upload->submit().timelineValue;
                }
                commandBuffer = VK_NULL_HANDLE;
            } else {
                if (vkEndCommandBuffer(commandBuffer) != VK_SUCCESS) {
                    throw std::runtime_error(
                        "Could not finish cubemap upload command buffer");
                }
                submitAndWait(device_, queue, commandBuffer);
            ...
            if (vkCreateSampler(device_, &sampler, nullptr, &sampler_) != VK_SUCCESS) {
                throw std::runtime_error(
                    "Could not create cubemap sampler");
            }
            if (upload == nullptr) {
    ...
    void Cubemap::destroy() noexcept {
        ...
        faceImageViews_.fill(VK_NULL_HANDLE);
