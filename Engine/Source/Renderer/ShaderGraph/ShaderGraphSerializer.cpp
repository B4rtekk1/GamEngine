...
namespace Engine {
    namespace {
        ...
        [[nodiscard]] std::optional<ShaderNodeType> shaderNodeTypeFromV1(const int type) {
            ...
            if (type == 20) {
                return ShaderNodeType::SurfaceOutput;
            }
            if (type < static_cast<int>(ShaderNodeType::Float) || type > static_cast<int>(
                    ShaderNodeType::SurfaceOutput)) {
                return std::nullopt;
            }
        ...
        void writeValue(std::ostream &output, const ShaderNodeValue &value) {
            if (std::holds_alternative<std::monostate>(value)) {
                ...
                output << " FLOAT " << *number;
            } else if (const auto *value2 = std::get_if<Vec2>(&value)) {
                output << " VEC2 " << value2->x() << ' ' << value2->y();
            } else if (const auto *value3 = std::get_if<Vec3>(&value)) {
                output << " VEC3 " << value3->x() << ' ' << value3->y() << ' ' << value3->z();
            } else if (const auto *value4 = std::get_if<Vec4>(&value)) {
                output << " VEC4 " << value4->x() << ' ' << value4->y() << ' ' << value4->z() << ' ' << value4->w();
            } else {
                output << " STRING " << std::quoted(std::get<std::string>(value));
            }
        }
        ...
        ShaderNodeValue readValue(std::istream &input, const std::filesystem::path &path) {
            ...
            float z{};
            float w{};
            if (tag == "FLOAT" && input >> x) {
                return x;
            }
            if (tag == "VEC2" && input >> x >> y) {
                return Vec2{x, y};
            }
            if (tag == "VEC3" && input >> x >> y >> z) {
                return Vec3{x, y, z};
            }
            if (tag == "VEC4" && input >> x >> y >> z >> w) {
                return Vec4{x, y, z, w};
            }
            std::string text;
            if (tag == "STRING" && input >> std::quoted(text)) {
                return text;
            }
            invalid(path);
        }
        ...
        ShaderPin readPin(std::istream &input, const std::filesystem::path &path) {
            int type{};
            ShaderPin pin;
            if (!(input >> pin.id >> type >> std::quoted(pin.name))) {
                invalid(path);
            }
    ...
    void ShaderGraphSerializer::save(const ShaderGraphAsset &graph, const std::filesystem::path &path) {
        ...
        if (!output) {
            throw std::runtime_error("Could not save shader graph: " + path.string());
        }
        ...
        for (const auto &node: graph.nodes) {
            ...
            for (const auto &pin: node.inputs) {
                writePin(output, "INPUT", pin);
            }
            for (const auto &pin: node.outputs) {
                writePin(output, "OUTPUT", pin);
            }
            output << "ENDNODE\n";
        }
        for (const auto &link: graph.links) {
            output << "LINK " << link.id << ' ' << link.fromPin << ' ' << link.toPin << '\n';
        }
        output << "END\n";
    }
    ...
    ShaderGraphAsset ShaderGraphSerializer::load(const std::filesystem::path &path) {
        ...
        if (!input) {
            throw std::runtime_error("Could not open shader graph: " + path.string());
        }
        std::string tag;
        int version{};
        if (!(input >> tag >> version) || tag != "SHADERGRAPH" || (version != 1 && version != 2)) {
            invalid(path);
        }
        ShaderGraphAsset graph;
        if (!(input >> tag >> graph.id >> std::quoted(graph.name)) || tag != "GRAPH") {
            invalid(path);
        }
        while (input >> tag) {
            if (tag == "END") {
            ...
            if (tag == "PROPERTY") {
                ...
                if (!(input >> property.id >> type >> std::quoted(property.name) >>
                      std::quoted(property.referenceName))) {
                    invalid(path);
                }
                ...
                float x{};
                float y{};
                if (!(input >> node.id >> typeId >> x >> y >> propertyId)) {
                    invalid(path);
                }
                const auto type = version == 1
                                      ? shaderNodeTypeFromV1([&] {
                                          try { return std::stoi(typeId); } catch (...) { invalid(path); }
                                      }())
                ...
                ShaderLink link;
                if (!(input >> link.id >> link.fromPin >> link.toPin)) {
                    invalid(path);
                }
