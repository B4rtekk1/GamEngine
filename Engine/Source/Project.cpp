...
namespace Engine {
    namespace {
        [[nodiscard]] std::string trim(std::string value) {
            ...
            const auto last = std::ranges::find_if_not(value.rbegin(), value.rend(), isSpace).base();
            ...
            for (const auto &part: path) {
                if (part == "..") {
                    throw std::runtime_error(
                        "Project key '" + std::string{key} + "' cannot leave the project directory");
                }
            }
    ...
    Project Project::create(const std::filesystem::path &rootDirectory, std::string name) {
        ...
        if (error) {
            throw std::runtime_error("Could not create project directories: " + error.message());
        }
        ...
        if (!manifestOutput) {
            throw std::runtime_error("Could not create project manifest: " + manifest.string());
        }
        manifestOutput << "# GamEngine project\nname = " << name
                << "\nasset_root = Assets\nstartup_scene = Assets/Scenes/Main.scene\n";
        if (!manifestOutput) {
            throw std::runtime_error("Could not write project manifest: " + manifest.string());
        }
    ...
    std::vector<std::filesystem::path> Project::scenes() const {
        ...
        for (std::filesystem::recursive_directory_iterator iterator{directory, error}, end;
            ...
            if (!iterator->is_regular_file(error) || iterator->path().extension() != ".scene") {
                continue;
            }
