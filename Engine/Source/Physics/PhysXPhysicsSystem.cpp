...
Vec3 absoluteScale(const Transform& transform) noexcept {
    return {std::max(std::abs(transform.scale.x()), MinimumDimension),
...
struct PhysicsSystem::BroadPhaseCache final {
    ...
    BroadPhaseCache() {
        ...
        if (foundation == nullptr) fail("PxCreateFoundation failed");

        PxTolerancesScale scale;
        scale.length = 1.0F;
        scale.speed = 9.81F;
        physics = PxCreatePhysics(PX_PHYSICS_VERSION, *foundation, scale, true, nullptr);
        if (physics == nullptr) fail("PxCreatePhysics failed");
        ...
        if (dispatcher == nullptr) fail("PxDefaultCpuDispatcherCreate failed");
        ...
        if (physicsScene == nullptr) fail("PxPhysics::createScene failed");
    }
    ...
    void releaseActors() noexcept {
        for (auto& record : actors | std::views::values) {
            if (record.actor != nullptr) record.actor->release();
        }
        actors.clear();
    }
    ...
    physx::PxConvexMesh* cookConvex(const std::vector<physx::PxVec3>& points) const {
        ...
        if (!PxCookConvexMesh(cookingParameters, description, output)) return nullptr;
    ...
    physx::PxTriangleMesh* cookTriangleMesh(const Mesh& mesh, const Vec3& scale) const {
        ...
        physx::PxDefaultMemoryOutputStream output;
        if (!PxCookTriangleMesh(cookingParameters, description, output)) return nullptr;
        physx::PxDefaultMemoryInputData input{output.getData(), output.getSize()};
    ...
    bool attachBoundsFallback(physx::PxRigidActor& actor, const Mesh& mesh,
        ...
        for (const Vertex& vertex : mesh.vertices) {
            minimum = {std::min(minimum.x(), vertex.position.x()),
                       std::min(minimum.y(), vertex.position.y()),
                       std::min(minimum.z(), vertex.position.z())};
            maximum = {std::max(maximum.x(), vertex.position.x()),
        ...
        return attachGeometry(actor, geometry, material,
            physx::PxTransform{toPhysX((center + collider.offset) * scale)},
    ...
    bool attachCollider(physx::PxRigidActor& actor, const ColliderComponent& collider,
        ...
        bool attached = std::visit([&](const auto& shape) {
            ...
            if constexpr (std::is_same_v<Shape, BoxCollider>) {
                ...
                const PxTransform capsulePose{localOffset.p,
                ...
                if (shape.mesh == nullptr || shape.mesh->vertices.empty()) return false;
                if (dynamic) {
    ...
    ActorRecord createActor(const Entity entity, Registry& owner, Transform& transform) const {
        ...
        RigidbodyComponent* body = owner.has<RigidbodyComponent>(entity)
            ? &owner.get<RigidbodyComponent>(entity) : nullptr;
    ...
    void pushEcsState(Registry& owner) {
        ...
        for (auto& [entity, record] : actors) {
            ...
            if (body.type == RigidbodyType::Dynamic) {
                if (nonZero(body.force)) rigid.addForce(toPhysX(body.force), PxForceMode::eFORCE);
                if (nonZero(body.torque)) rigid.addTorque(toPhysX(body.torque), PxForceMode::eFORCE);
    ...
    void pullPhysXState(Registry& owner) {
        for (auto& [entity, record] : actors) {
            if (!owner.has<RigidbodyComponent>(entity)) continue;
            auto& body = owner.get<RigidbodyComponent>(entity);
            ...
            if (body.fixedRotation) body.angularVelocity = {};
            record.lastTransform = transform;
            record.lastLinearVelocity = body.linearVelocity;
            record.lastAngularVelocity = body.angularVelocity;
...
std::optional<RaycastHit> PhysicsSystem::raycast(
    ...
    return RaycastHit{Actor{scene, object->objectId()}, fromPhysX(hit.block.position),
