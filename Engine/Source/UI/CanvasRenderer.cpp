...
namespace Engine::UI {
    ...
    void CanvasRenderer::sortIfNeeded(const std::vector<const UIElement *> &source,
        ...
        cache.source = source;
        cache.sorted = source;
        std::ranges::stable_sort(cache.sorted,
    ...
    bool CanvasRenderer::ensureCapacity(FrameResources &frame) const {
    ...
    void CanvasRenderer::record(const Canvas &canvas,
        ...
        FrameResources &frame = *frames_.at(frameIndex.value);
        if (!batch_.empty()) {
            const bool recreated = ensureCapacity(frame);
            const std::uint64_t frameBit = std::uint64_t{1} << frameIndex.value;
        ...
        pipeline_.record(commandBuffer, imageIndex.value, extent,
