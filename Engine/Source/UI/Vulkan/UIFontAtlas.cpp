...
namespace Engine::UI {
    namespace {
        ...
        public:
            bool load(const std::string &path, std::string &error) {
                ...
                units_ = u16(head.offset + 18);
                indexFormat_ = s16(head.offset + 50);
                glyphCount_ = u16(maxp.offset + 4);
                ascender_ = s16(hhea.offset + 4);
                descender_ = s16(hhea.offset + 6);
                lineGap_ = s16(hhea.offset + 8);
                metricCount_ = u16(hhea.offset + 34);
                loca_ = loca;
                glyf_ = glyf;
                hmtx_ = hmtx;
            ...
            std::uint16_t glyph(std::uint32_t cp) const {
                if (cmapFormat_ == 4) {
                    for (std::size_t i = 0; i < segments_.size(); ++i) {
                        if (cp <= segments_[i].end) {
                            if (cp < segments_[i].start) {
                                return 0;
                            }
                            const auto delta = segments_[i].delta;
                            if (segments_[i].rangeOffset == 0) {
                            ...
                            const auto id = u16(at);
                            return id == 0 ? 0 : static_cast<std::uint16_t>((id + delta) & 0xffffU);
                        }
                    }
                }
                return 0;
            ...
            bool outline(std::uint16_t id, Outline &out) const {
                ...
                const auto off = glyphOffset(id);
                const auto next = glyphOffset(id + 1);
                if (next <= off || glyf_.offset + next > data_.size()) {
                    return true;
                }
                const auto p = glyf_.offset + off;
                const auto contours = s16(p);
                if (contours < 0) {
                    return false; // Composite glyphs are intentionally skipped for this small rasterizer.
                }
                if (contours == 0) {
                    return true;
                }
                ...
                decodeCoordinates(flags, pos, xs, true);
                decodeCoordinates(flags, pos, ys, false);
                std::size_t first = 0;
        ...
        private:
            ...
            Table table(const char *name) const {
                ...
                return i == tables_.end() ? Table{} : i->second;
            }
            ...
            void decodeCoordinates(const std::vector<std::uint8_t> &flags, std::size_t &pos,
                                   std::vector<std::int16_t> &values, bool x) const {
                std::int32_t value = 0;
            ...
            bool readCmap(Table cmap, std::string &error) {
                ...
                for (std::uint16_t i = 0; i < n; ++i) {
                    ...
                    if ((platform == 3 && (encoding == 1 || encoding == 10)) || platform == 0) {
                        if (u16(cmap.offset + off) == 4) {
                            chosen = cmap.offset + off;
                            break;
                        }
                    }
                }
    ...
    std::string UIFontAtlas::build(const std::string &fontPath, std::uint32_t pixelSize, std::uint32_t firstCodepoint,
        ...
        row = 0;
        for (auto &q: pending) {
            const auto w = static_cast<std::uint32_t>(q.glyph.width);
            const auto h = static_cast<std::uint32_t>(q.glyph.height);
            if (x + w + padding > atlasWidth) {
                x = padding;
                y += row + padding;
                row = 0;
                ...
                for (std::uint32_t px = 0; px < w; ++px) {
                    ...
                    for (std::uint32_t sy = 0; sy < samples; ++sy) {
                        for (std::uint32_t sx = 0; sx < samples; ++sx) {
                            const double fx = ((px + ((sx + .5) / samples)) / scale) + (q.glyph.bearingX / scale);
                            const double fy = (q.glyph.bearingY / scale) - ((h - py - ((sy + .5) / samples)) / scale);
                            sum += coverage(q.outline.contours, fx, fy);
                        }
                    }
                    m_pixels[std::size_t(y + py) * m_width + x + px] = static_cast<std::uint8_t>(
                        sum * 255 / (samples * samples));
                }
            q.glyph.uv.min[1] = static_cast<float>(y) / m_height;
            q.glyph.uv.max[1] = static_cast<float>(y + h) / m_height;
            m_glyphs.emplace(q.cp, q.glyph);
            x += w + padding;
            row = std::max(row, h);
        }
        return {};
