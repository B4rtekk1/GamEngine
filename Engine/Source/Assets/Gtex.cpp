...
namespace Engine::Assets {
    ...
    bool valid_cooked_texture(const CookedTexture& texture) noexcept {
        if (texture.width == 0 || texture.height == 0 || texture.mips.empty() || !supported_format(texture.format)) return false;
        std::uint32_t expectedWidth = texture.width;
        std::uint32_t expectedHeight = texture.height;
        for (const TextureMip& mip : texture.mips) {
            if (mip.width != expectedWidth || mip.height != expectedHeight || mip.size != mip_size(texture.format, mip.width, mip.height) ||
                mip.offset > texture.data.size() || mip.size > texture.data.size() - mip.offset) return false;
    ...
    std::optional<CookedTexture> load_gtex(const std::filesystem::path& path) {
        ...
        if (!stream || !std::equal(kMagic.begin(), kMagic.end(), header.magic) || header.version != kVersion ||
            header.mipCount == 0 || header.mipCount > 32 || header.format > static_cast<std::uint32_t>(TextureFormat::BC7_SRGB)) return std::nullopt;
        ...
        if (!stream) return std::nullopt;
        CookedTexture texture{header.width, header.height, static_cast<TextureFormat>(header.format)};
        texture.mips.reserve(entries.size());
        const auto dataSize = static_cast<std::size_t>(static_cast<std::uint64_t>(length) - headerBytes);
        texture.data.resize(dataSize);
        stream.read(reinterpret_cast<char*>(texture.data.data()), static_cast<std::streamsize>(dataSize));
        if (!stream) return std::nullopt;
        for (const FileMip& entry : entries) texture.mips.push_back({entry.offset, entry.size, entry.width, entry.height});
        return valid_cooked_texture(texture) ? std::optional<CookedTexture>{std::move(texture)} : std::nullopt;
    }

    bool save_gtex(const std::filesystem::path& path, const CookedTexture& texture) {
        if (!valid_cooked_texture(texture) || texture.mips.size() > std::numeric_limits<std::uint32_t>::max()) return false;
        std::ofstream stream(path, std::ios::binary | std::ios::trunc);
        if (!stream) return false;
        const FileHeader header{{'G', 'T', 'E', 'X'}, kVersion, texture.width, texture.height,
        ...
        stream.write(reinterpret_cast<const char*>(texture.data.data()), static_cast<std::streamsize>(texture.data.size()));
