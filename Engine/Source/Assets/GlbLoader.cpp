    ...
    namespace {
        ...
        [[nodiscard]] std::int32_t named_quixel_image(const std::vector<NamedImage> &images,
            ...
            for (const NamedImage &image: images) {
                ...
                if (!singleMaterialAsset &&
                    (materialLabel.empty() || image.label.find(materialLabel) == std::string::npos))
                    continue;
        ...
        void load_materials(const cgltf_data &data, const std::vector<NamedImage> &images, Mesh &mesh) {
            ...
            for (cgltf_size i = 0; i < data.materials_count; ++i) {
                ...
                if (source.has_pbr_metallic_roughness) {
                    ...
                    import_texture_transform(material, MaterialTextureSlot::MetallicRoughness,
                ...
                if (material.opacityTexture < 0)
                    material.opacityTexture = named_quixel_image(images, source, data.materials_count == 1,
                                                                 {"opacity", "_op."});
                if (material.translucencyTexture < 0)
                    material.translucencyTexture = named_quixel_image(images, source, data.materials_count == 1,
                                                                      {"translucency", "_trans."});
                if (material.translucencyTexture >= 0 && material.translucency == 0.0F)
                    material.translucency = 1.0F;
                material.displacementTexture = named_quixel_image(images, source, data.materials_count == 1,
                                                                  {"displacement", "height", "_disp."});
                if (material.specularTexture < 0)
                    material.specularTexture = named_quixel_image(images, source, data.materials_count == 1,
                                                                  {"specular", "_spec."});

                material.alphaMode = source.alpha_mode == cgltf_alpha_mode_mask
                                         ? AlphaMode::Mask
                ...
                const bool foliage = is_foliage_label(source) || material.opacityTexture >= 0 ||
                                     (material.doubleSided && (
                                          material.alphaMode == AlphaMode::Mask || material.alphaMode ==
                                          AlphaMode::Blend));
                if (foliage) {
        ...
        [[nodiscard]] bool append_primitive(const cgltf_data &data, const cgltf_node &node,
            ...
            if ((normals != nullptr && normals->count != positions->count) ||
                ...
                (sourceTangents != nullptr && sourceTangents->count != positions->count))
                return false;
            ...
            for (cgltf_size i = 0; i < positions->count; ++i) {
                ...
                if (normals != nullptr) {
                    ...
                    const glm::vec3 normal = glm::normalize(
                        normalTransform * glm::vec3{values[0], values[1], values[2]});
            ...
            for (std::size_t i = 0; i < localIndices.size(); i += 3) {
                ...
                if (orientation > 0.0F) {
                    mesh.indices.insert(mesh.indices.end(), {
                                            offset + localIndices[i], offset + localIndices[i + 1],
                                            offset + localIndices[i + 2]
                                        });
                } else {
                    mesh.indices.insert(mesh.indices.end(), {
                                            offset + localIndices[i], offset + localIndices[i + 2],
