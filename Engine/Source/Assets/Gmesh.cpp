...
std::shared_ptr<const Mesh> load_gmesh(const std::filesystem::path& path) {
    ...
    for (auto& image : mesh.images) { std::uint32_t length{}; if (!read(file, length) || length > 32'768) return {}; std::string text(length, '\0'); file.read(text.data(), length); if (!file) return {}; image.cookedPath = text; const auto cooked = load_gtex(path.parent_path() / image.cookedPath); if (!cooked) return {}; image.width = cooked->width; image.height = cooked->height; image.cooked = *cooked; }
