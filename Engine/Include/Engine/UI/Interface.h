...
namespace Engine::UI {
    ...
    inline PanelElement &Interface::panel(std::string_view name, ElementOptions options) {
        return add(std::make_unique<PanelElement>(options.color), options);
    }

    inline TextElement &Interface::label(std::string_view text, const ElementOptions &options) {
    ...
    inline void Interface::dispatch(UIElement &element, const Vec2 &mousePosition) {
        if (element.visible) {
            ...
            if (inside) {
                if (auto *button = dynamic_cast<ButtonElement *>(&element)) {
                    button->click();
                }
            }
        }
        for (const auto &child: element.children()) {
            dispatch(*child, mousePosition);
        }
    }

    inline void Interface::update() const {
        if (!Input::mousePressed(MouseButton::Left)) {
            return;
        }
        const Vec2 position = Input::mousePosition();
        for (const auto &element: canvas_->elements()) {
            dispatch(*element, position);
        }
    }
} // namespace Engine::UI
