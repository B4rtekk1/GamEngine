...
namespace Engine {
    ...
    struct AABB {
        ...
        [[nodiscard]] static constexpr AABB unitCube() noexcept {
            return {{-0.5F, -0.5F, -0.5F}, {0.5F, 0.5F, 0.5F}}; //NOLINT
        }
        ...
        [[nodiscard]] AABB transformed(const glm::mat4 &transform) const noexcept {
            ...
            for (const float x: {min.x(), max.x()}) { //NOLINT
                for (const float y: {min.y(), max.y()}) { //NOLINT
                    for (const float z: {min.z(), max.z()}) { //NOLINT
