...
bool ComponentsPanel::draw(Engine::ScenePreset &scene, Engine::Assets::Content& content,
    ...
    if (scene.editor().valid(selected) && scene.editor().has<Engine::MeshRenderer>(selected)) {
        ...
        if (remove) {
            if (scene.editor().has<Engine::WaterBodyComponent>(selected)) {
                // A water archetype has no independently-removable mesh.
                // Retire the complete entity after frame submission.
    ...
    if (scene.editor().valid(selected) && scene.editor().has<Engine::WaterBodyComponent>(selected)) {
        ...
        if (remove) {
            ...
            const int type = static_cast<int>(water.type);
            ImGui::TextDisabled("Water type: %s", types[type]);
            ImGui::TextDisabled("Create a different Water archetype to change type.");
            bool changed = false;
            ...
            ImGui::TextDisabled("Geometry is authored as Water Shape and generated by Virtual Water.");
        }
    }
